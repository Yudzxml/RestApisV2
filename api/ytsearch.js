...
const fetchVideoById = async (videoId) => {
  const res = await yts({ query: videoId });
  if (!res?.videos?.length) throw new Error('Video not found.');
...
const fetchChannelUploads = async (channelId) => {
  const res = await yts({ query: `channel:${channelId}` });
  if (!res?.videos?.length) throw new Error('Channel or uploads not found.');
  return res.videos.slice(0, 5).map((v) => ({
    title: v.title,
    videoUrl: v.url,
    thumbnailUrl: v.thumbnail,
    duration: v.duration,
...
const searchYouTube = async (query) => {
  ...
  if (!res?.videos?.length) throw new Error('No videos found.');
  const v = res.videos[0];
  return {
    title: v.title,
    videoUrl: v.url,
